...
class SwingApiTest extends FlatSpec {
  ...
  "SwingApi" should "produce text field values" in {
    val textField = new swingApi.TextField
    val values = swingApi.textFieldValues(textField)
  ...
  it should "produce the clicks correctly" in {
    val button = new swingApi.Button
    val clicks = swingApi.buttonClicks(button)
    ...
    assert(observed == Seq("click", "click", "click", "click"), observed)
  }
